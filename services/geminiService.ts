...
export const extractReceiptData = async (base64Image: string, mimeType: string): Promise<ReceiptData> => {
  ...
  const prompt = `
    ...
    - 8%対象税込合計（total8Amount）: 軽減税率（8%）が適用される商品の税込合計金額を数値で読み取ること。該当がない場合は0とすること。
    - 10%対象税込合計（total10Amount）: 標準税率（10%）が適用される商品の税込合計金額を数値で読み取ること。該当がない場合は0とすること。
    ...
    config: {
      ...
      responseSchema: {
        ...
        properties: {
          ...
          taxAmount: { type: Type.NUMBER, description: "消費税額の総額。記載なけれで0" },
          total8Amount: { type: Type.NUMBER, description: "8%対象商品の税込合計金額。記載なけれで0" },
          total10Amount: { type: Type.NUMBER, description: "10%対象商品の税込合計金額。記載なけれで0" },
